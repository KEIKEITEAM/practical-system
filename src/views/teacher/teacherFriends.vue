<template>
  <div id="nv">
    <div class="userbox">
      ...
      <div class="userbox-1">
        <!-- 教师个人信息 -->
        <van-cell size="large" title="工号：" icon="user-o">{{teacherData.teacherNumber}}</van-cell>
        <van-cell size="large" title="姓名：" icon="user-o">{{teacherData.name}}</van-cell>
        <van-cell size="large" title="专业群：" icon="cashier-o">{{teacherData.professional.name}}</van-cell>
        <van-cell size="large" title="电话号码：" icon="phone-o">{{teacherData.tel}}</van-cell>
        <van-cell size="large" title="qq:" icon="user-circle-o">{{teacherData.qq}}</van-cell>
        <!-- 教师个人信息 end-->
  ...
  methods: {
    ...
    getTeacherInformation() {
      this.$axios.get("/teacher").then(response => {
        ...
        if (msg.code == 0) {
          this.teacherData = msg.data;
        } else {
          this.$toast(msg.msg);
        }
      });
    },
    //退出当前用户登录（点击退出按钮后弹出提示框）
    logout() {
      this.$dialog
        .confirm({
          title: "提示",
          message: "确定要退出当前用户登录吗？"
        })
        .then(() => {
          //选择确认按钮后执行
          this.logo();
    ...
    //退出当前用户登录
    logo() {
      this.$axios.get("/teacher/logout").then(response => {
        let msg = response.data;
        if (msg.code == 0) {
    ...
    submitUpdateForm() {
      ...
      if (validate) {
        ...
        //提交表单事件
        this.$axios
          .put("/teacher/updatePassword", JSON.stringify(this.updateform))
          .then(response => {
            //获取返回数据
            let msg = response.data;
            console.log(response.data);
    ...
    submitUpdatexx() {
      this.$axios
        .put("/teacher", JSON.stringify(this.updatexxform))
        .then(response => {
          let msg = response.data;
          if (msg.code == 0) {
            //如果修改信息成功
